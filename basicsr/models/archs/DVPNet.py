...
class frequenctSpecificPromptGenetator(nn.Module):
    def __init__(self, dim=3,h=128,w=65):
        super().__init__()
        k_size = 3
    ...
    def forward(self, ffm, H, W):
        ffm = F.interpolate(ffm, size=(self.h, self.w), mode='bicubic',align_corners=False)
        ...
        y = y * weight
        y = torch.fft.irfft2(y, s=(self.h, self.w))
...
class PromptFFTModule(nn.Module):
    def __init__(self, basic_dim=32, dim=32, input_resolution=128):
        super().__init__()
        h = input_resolution
        w = input_resolution // 2 + 1
        ...
        self.FFT_feature = frequenctSpecificPromptGenetator(basic_dim, h, w)

        self.modulator_fe = FrequencyPromptFusion(dim, basic_dim, win_size=8, num_heads=2, bias=False)

    def forward(self, x_select, x):
        b, c, h, w = x.shape

        FFT_feature_t = self.FFT_feature(x_select, h, w)

        y_m = self.modulator_fe(x, FFT_feature_t)

        # x = self.simple_Fusion(torch.cat([y_m,x ], dim=1))

        return y_m

## PromptModule
class splitFrequencyModule(nn.Module):
    def __init__(self, basic_dim=32, dim=32):
        super().__init__()
...
## PromptModule
class PromptFeatureSelect(nn.Module):
    def __init__(self, basic_dim=32, dim=32,
                 ffn_expansion_factor=3.0, bias=False, LayerNorm_type='WithBias'):
        super().__init__()
        self.in_c = nn.Conv2d(dim, basic_dim, kernel_size=1, bias=bias)
        self.norm2 = LayerNorm(basic_dim, LayerNorm_type)
        self.featureselect = FeedForward(basic_dim, ffn_expansion_factor, bias)

    def forward(self, x):
        x = self.in_c(x)
        x = self.featureselect(self.norm2(x))
...
class DVPNet(nn.Module):
    def __init__(self, 
        ...
        super(DVPNet, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

        self.encoder_level1 = nn.Sequential(*[TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, isAtt=True) for i in range(num_blocks[0])])
        
        self.down1_2 = Downsample(dim) ## From Level 1 to Level 2
        self.encoder_level2 = nn.Sequential(*[TransformerBlock(dim=int(dim*2**1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, isAtt=True) for i in range(num_blocks[1])])
        
        self.down2_3 = Downsample(int(dim*2**1)) ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[TransformerBlock(dim=int(dim*2**2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, isAtt=True) for i in range(num_blocks[2])])

        self.PromptFeatureSelect =PromptFeatureSelect(basic_dim= dim, dim = dim*2**2, ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type)
        ...
        self.prompt_d3 = PromptFFTModule(basic_dim= dim,dim=int(dim*2**2),input_resolution=64)
        ...
        self.PromptFeatureSelect2 = PromptFeatureSelect(basic_dim=dim, dim=dim*2**1, ffn_expansion_factor=ffn_expansion_factor,
                                                       bias=bias, LayerNorm_type=LayerNorm_type)
        self.prompt_d2 = PromptFFTModule(basic_dim= dim,dim=int(dim*2**1),input_resolution=128)
        ...
        self.PromptFeatureSelect1 = PromptFeatureSelect(basic_dim=dim, dim = dim, ffn_expansion_factor=ffn_expansion_factor,
                                                       bias=bias, LayerNorm_type=LayerNorm_type)
        self.prompt_d1 = PromptFFTModule(basic_dim= dim,dim=int(dim*2**1),input_resolution=256)
    ...
    def forward(self, inp_img):
        ...
        selected_features3 = self.PromptFeatureSelect(inp_enc_level3)
        out_dec_level3 = self.prompt_d3(selected_features3,out_dec_level3) + out_dec_level3

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2 = self.decoder_level2(inp_dec_level2)
        selected_features2 = self.PromptFeatureSelect2(inp_enc_level2)
        out_dec_level2 = self.prompt_d2(selected_features2,out_dec_level2) + out_dec_level2

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        out_dec_level1 = self.decoder_level1(inp_dec_level1)
        selected_features1 = self.PromptFeatureSelect1(inp_enc_level1)
        out_dec_level1 = self.prompt_d1(selected_features1,out_dec_level1) + out_dec_level1
